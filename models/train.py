...
import numpy as np
import os
from models.rep import save_representation_resnet, freezeTraining, freezeFCN

class Trainer():
    def __init__(self,model,training_loader,optimizer,criterion,validation_loader=None,test_loader=None,verbose=False,nb_classes = 0,
                 saving_folder=None,nb_outputs=1,save_best=False,f1_macro=True, rep_data = None, rep_saving_folder = None,dataset=None):
        self.model = model
        ...
        self.rep_saving_folder = rep_saving_folder
        self.freezed_layers = []
    ...
    def train(self,nb_epochs,drop_learning_rate=[],name='',save_cca = [], freeze_rate = 0.95, freeze = False, begin_freeze = 30, check_epoch=[]):
        ...
        for epoch in range(starting_epoch,nb_epochs):
            if epoch in drop_learning_rate:
                for g in self.optimizer.param_groups:   
                    print('Drop lr by 0.1')
                    g['lr'] = g['lr'] * 0.1

            
            if epoch in save_cca:
                act = save_representation_resnet(self.model, epoch, tqdm(self.rep_data), self.device)
                    for idx in act.keys():
                        torch.save(act[idx], 'Epoch' + str(epoch) + self.saving_folder + idx + '.pt')
                    
            if freeze and epoch in check_epoch:
                print("Check similarity...")
                self.freezed_layers += freezeTraining(epoch-1, epoch, self.model, self.dataset, freeze_rate)
                
            if epoch % 10 == 0 and len(self.freezed_layers) != 0:
                for i in self.freezed_layers:
                    print(i)
                print("are freezed")
